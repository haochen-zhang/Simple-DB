...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (pid.pgcateg() == BTreePageId.INTERNAL) {
            ...
            if (it.hasNext() && Objects.isNull(f)) {
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        if (Objects.nonNull(page.getRightSiblingId())) {
        ...
        assert tuple != null;
        Field key = tuple.getField(this.keyField);
        ...
        return field.compare(Op.GREATER_THAN_OR_EQ, key) ? newPage : page;
    }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        parent.insertEntry(entry);
        updateParentPointers(tid, dirtypages, parent);
        return key.compare(Op.GREATER_THAN_OR_EQ, field) ? page : newPage;
    }
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreeEntry entry = parentEntry;
        BTreePageId rightChild = page.iterator().next().getLeftChild();
        BTreeEntry key = null;
        while(it.hasNext() && numToShare > 0) {
            key = new BTreeEntry(entry.getKey(), null, rightChild);
            entry = it.next();
            key.setLeftChild(entry.getRightChild());
            page.insertEntry(key);
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreeEntry key = null;
        while(it.hasNext() && numToShare > 0) {
            key = new BTreeEntry(entry.getKey(), leftChild, null);
            entry = it.next();
            key.setRightChild(entry.getLeftChild());
            page.insertEntry(key);
    ...
    public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        if (Objects.nonNull(rightPage.getRightSiblingId())) {
            BTreeLeafPage descendant = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
            descendant.setLeftSiblingId(leftPage.getId());
        }
    ...
    public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // Delete the entry in the parent corresponding to the two pages that are merging -
        // deleteParentEntry() will be useful here
        BTreeEntry key = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
        leftPage.insertEntry(key);
