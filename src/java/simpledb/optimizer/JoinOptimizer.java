...
//                String table1Name = Database.getCatalog().getTableName(tableAliasToId.get(table1Alias));
//                String table2Name = Database.getCatalog().getTableName(tableAliasToId.get(table2Alias));
//                int range1 = stats.get(table1Name).estimateTableCardinality(stats.get(table1Name).avgSelectivity(field1PureName, joinOp));
//                int range2 = stats.get(table2Name).estimateTableCardinality(stats.get(table2Name).avgSelectivity(field2PureName, joinOp));
                if (!t1pkey && !t2pkey) {
                    card = Math.max(Math.max(card1, card2), (int) Math.ceil(card1 * card2 * 0.3 * 0.3));
                } else {
    ...
    public List<LogicalJoinNode> orderJoins(
        ...
        CostCard planCostCard = null;
        for(int i = 1; i <= joins.size(); i++) {
            Set<Set<LogicalJoinNode>> joinSetCollection = enumerateSubsets(joins, i);
            for(Set<LogicalJoinNode> joinSet : joinSetCollection) {
                ...
                if(!Objects.isNull(bestCostCard)) {
                    pc.addPlan(joinSet, bestCostCard.cost, bestCostCard.card, bestCostCard.plan);
                    planCostCard = bestCostCard;
                }
            }
        }
        assert(!Objects.isNull(planCostCard));
        if (explain) {
            printJoins(planCostCard.plan, pc, stats, filterSelectivities);
        }
        return planCostCard.plan;
    }
